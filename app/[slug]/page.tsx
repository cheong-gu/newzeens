...
export default async function Page({ params: { slug } }: DetailPageProps) {
  const newsletterInfo = await getCurrentNewsletter(slug);
  const featuredList = await getFeaturedNewsletter(slug, newsletterInfo.field);
